...
public partial class transactiondetails : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string id = Request.QueryString["RELATIONSHIPID"];
            ViewState["relationshipid"] = id;
            if (id != null)
            {
                string today = DateTime.Now.ToString("dd/MM/yyyy");
                txtfromdate.Text = today;
                txttodate.Text = today;
                FillData(today, today, id);
            }
    ...
    private void FillData(string fromdate, string todate, string id)
    {
    ...
    private string GetOpeningAmount(string fromdate,string id)
    {
        double credit = 0;
        double debit = 0;
        double amount = 0;
    ...
    protected void btnshow_Click(object sender, EventArgs e)
    {
        ...
        if (ViewState["relationshipid"].ToString().Trim() != string.Empty)
        {
            FillData(txtfromdate.Text.Trim().ToString(), txttodate.Text.Trim().ToString(), ViewState["relationshipid"].ToString().Trim());
        }
    }
